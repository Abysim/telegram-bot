...
class HelpCommand extends UserCommand
{
     ...
     * @var string
     */
    protected $description = 'Показати список команд бота';
    ...
    public function execute(): ServerResponse
    {
        ...
        if ($command_str === '') {
            ...
            if ($safe_to_show && count($admin_commands) > 0) {
                $text .= PHP_EOL . '*Список адмінських команд*:' . PHP_EOL;
                foreach ($admin_commands as $admin_command) {
                    $text .= '/' . $admin_command->getName() . ' - ' . $admin_command->getDescription() . PHP_EOL;
                }
            }

            $text .= PHP_EOL . 'Для довідки про певну кманду набери: /help <команда>';
        ...
        return $this->replyToChat('Довідка не доступна: команду `/' . $command_str . '` не знайдено', ['parse_mode' => 'markdown']);
    }
